...
need to change this to run on your own machine. You may need to put an
'r' on the beginning of the string for it to run on Windows (eg
r"C:/blah/Documents...")

Within document_home, the acquisitions database in stored in a folder
...
3 seconds for all the data to load, then run the function run(). This
will take a reasonably long time - it should print out updates every
minute or so, but if it takes more than half an hour or so, there may
be a problem going unreported. You can vary the similarity threshold
by supplying it with an argument (eg run(threshold=1.4)). I found a
threshold 0.9 worked reasonably well, but if you start finding matches
which you think are not correct, you can raise it. Similarly, if you
think some matches may be missed, you can lower it, but I found that
at around 0.85 and below you begin to get matches which are clearly
wrong.

This gives a file Loans.xlsx.

To put all the Syndicated Loan databases together, run
make_big_loans_sheet(). This also gives them an ID which corresponds
to the ID in Loans.xlsx. This will take a very long time and eat all
your memory - it's best to leave this to run overnight and then
restart your computer in the morning. This makes a file Combined_Loans.xlsx, which will be incredibly huge.
"""
                ...
                for j, part in enumerate(loan.parts):
                    ...
                    for cp in comparisons[part.name]:
                        ...
                        try:
                            ...
                            if (match[1] and match[1] > loan.date):
...
def make_big_loans_sheet():
    ...
    columns = []
    sheet = xlrd.open_workbook(loans_sheets_paths[0]).sheet_by_index(0)
    
    newsheet.cell(row=1, column=1).value = "ID"
    for i in range(sheet.ncols):
        columns.append(sheet.cell_value(rowx=2, colx=i))
    ...
    for n, p in enumerate(loans_sheets_paths):
        ...
        for i in range(3, sheet.nrows):
            sleep(0.001)
            newsheet.cell(row = num, column = 1).value = num - 2
            for j in range(sheet.ncols):
                newsheet.cell(row = num, column = j + 2).value = sheet.cell_value(rowx = i, colx = j)
            num += 1
        print("Copied over sheet " + str(n) + " of " + str(len(loans_sheets_paths)))

    print("Saving database")
    wb.save("Combined_Loans.xlsx")
